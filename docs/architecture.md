...
| 平台端 | 品类、槽位、商家状态、全量数据 | POC JWT 要求 `platform` 角色；产品化时再扩展细粒度权限 |
...
1. 当前 POC 使用既有账号验证密码并签发短期 JWT，按 `customer`、`merchant`、`platform` 三类角色保护 HTTP 和 WebSocket；仍不包含 refresh token、服务端注销、多设备会话和细粒度权限。
