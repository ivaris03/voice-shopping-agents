...
- 平台端通过一张 `categories` 表维护一级分类、二级分类，以及每个二级分类的必填槽位和选填槽位。
- 商家创建商品时必须选择表中已有的二级分类，一级分类由同一行配置确定，不允许自由填写不一致的组合。
- 必填槽位必须提供非空且类型正确的值；选填槽位可以省略。未在该二级品类定义的槽位不允许提交。
- `required_slots` 与 `optional_slots` 分别保存槽位 Key 列表；前端据此动态生成表单，API 在写入数据库前再次执行相同约束校验。
