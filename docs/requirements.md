...
- 商品点击和成功订单更新动态画像，成功订单的品类/品牌增量高于点击。
- 对话中的年龄、城市、身高、体重、肤质、技术熟练度和预算等候选资料，先累积在本轮会话
  的 `user_profile_updates` 中；订单进入终态、用户显式关闭会话或页面关闭时，统一合并回写
  `user_profile_static`。显式渠道字段优先于对话抽取结果，空值不覆盖已有资料。
