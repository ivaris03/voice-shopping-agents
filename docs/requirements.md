...
- 推荐结果先展示商品卡，再为 Top 3 批量生成逐商品推荐理由和语音话术。
...
| Agent/节点 | 职责 |
| --- | --- |
| 意图识别 Agent | 根据当前话语和最近 3 轮对话识别一个主意图、置信度、订单 action 和标准化二级品类；抽取、校验并写入当前槽位 |
| 需求澄清 Agent | 检查当前品类必填槽位是否齐全；缺失时返回 pending question，每轮最多追问两个槽位，不填写槽位 |
...
3. 意图 Agent 结合用户当前话语、待回答问题和历史槽位抽取并合并槽位；只有通过槽位定义校验的值才可以写入状态。
4. 每轮最多询问一到两个缺失槽位；用户可以只回答其中一个。
5. 澄清 Agent 不抽取或修改槽位；所有必填槽位完成后返回 `READY`，进入商品推荐 Agent。
6. `budgetMax` 是跨品类通用的可选预算上限，只在当前会话槽位中保留并参与商品价格过滤；它不计入品类必填槽位，也不写入静态画像。
...
- 批量理由调用失败时整批降级；调用成功后逐卡检查，单卡命中禁用规则时只降级该卡。
- 文本增量推送前检查，不合规增量不得发送。
...
4. `ProfileReranker` 在完整 Top 20 候选池上叠加品牌偏好、历史购买和预算敏感度。
5. 按“向量分 + 画像分”重排后截断 Top 3，输出商品卡、`matchScore` 和 `scoreBreakdown`。
6. Top 3 一次性进入 LLM 批量生成推荐理由；模型失败时整批使用确定性理由降级。
...
- 静态画像：性别、年龄、城市、身高、体重、肤质、技术熟练度和 locale。
