...
本文定义第一版语音导购电商 MVP 的业务范围、Agent 契约和验收口径。当前代码已经落地的实现约束也在本文中明确，避免需求、架构和实现说明出现不同解释。

## 1. 项目目标

建设一个带语音导购能力的电商平台。用户可以浏览店铺和商品，也可以通过文字或语音完成需求澄清、商品推荐、商品查询、商品对比和下单。项目包含用户端、商家端、平台端三个独立前端，以及一个 FastAPI 模块化单体后端。
...
- MVP 使用请求头演示身份，业务 SQL 仍执行数据归属校验；生产版必须替换为真实认证和角色授权。
...
| 合规检查节点 | 按短句检查推荐理由和话术，命中禁用规则时路由到违规回复节点 |
...
推荐理由和完整话术不得包含绝对化、医疗功效、收益承诺或其他业务禁用表达。当前第一版以正则规则实现：
...
当前实现中，情感应答节点只生成完整话术；`compliance_check` 按 TTS 短句边界逐一检查，全部通过后才由 `publish_response` 调度文本增量和 TTS。命中禁用规则时路由到 `violation_response`，原始违规话术不会继续发布。
...
推荐卡必须只引用后端商品事实，包括商品 ID、商家、名称、品牌、价格、库存、图片、卖点和属性。推荐 Agent 不生成推荐理由。
...
订单状态只有 `pending`、`success`、`fail`。终态订单不可重新打开；重复请求按幂等键或当前订单状态返回，不能重复扣库存。第一版不包含支付。
...
包含：三个 Vue 前端、FastAPI HTTP API、六类单意图、动态品类槽位、四类导购 Agent 加订单/合规节点、双 WebSocket、ASR/TTS 适配、PGVector 推荐、用户画像、语音下单、三端订单查询、LangGraph Checkpointer 和可选 LangSmith Trace。

暂不包含：真实登录和角色授权、支付、退款、物流、售后、真实电商平台对接、复杂画像模型训练、后台异步向量重建任务和生产级限流告警体系。
