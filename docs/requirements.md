...
| 商家端 | 自有店铺 CRUD、在平台二级品类下维护商品、查看本店订单 |
| 平台端 | 维护两级品类及槽位、查看全部商家和商品、启用/禁用商家、查看全部订单 |
...
1. 查询平台维护的二级品类槽位，将其中标记为必填的 Key 作为 `requiredSlots`，选填槽位不阻塞澄清流程。
2. 从用户当前话语和后续回答中抽取并合并槽位。
3. 存在缺失项时返回 `ASK`，每轮追问一到两个槽位。
4. 所有必填槽位填写完成后返回 `READY`，进入商品推荐 Agent。

`budgetMax` 是跨品类通用的可选过滤条件，不计入品类必填 Key。

### 3.2 商品建品约束

- 平台端可独立创建一级分类；创建二级分类时必须关联一个已存在的一级分类。
- 二级分类创建完成后单独创建槽位。每个槽位必须标记为必填或选填，并同时提供至少一个
  非空枚举值；不允许创建没有枚举值的裸槽位。
- 商家创建商品时必须选择表中已有的二级分类，一级分类由同一行配置确定，不允许自由填写不一致的组合。
- 必填槽位必须提供枚举范围内的非空值；选填槽位可以省略，但填写时同样必须来自枚举。
  未在该二级品类定义的槽位不允许提交。
- 商家端根据槽位枚举动态生成选择控件，API 在写入数据库前再次执行相同约束校验。
