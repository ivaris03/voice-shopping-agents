...
-- Database: voice_shopping_agents
-- 创建数据库：CREATE DATABASE voice_shopping_agents;
-- 执行建表：psql -d voice_shopping_agents -f sql/schema.sql
...
-- 旧版目录页曾把浏览器本地生成的会话 ID 写入订单。它们并不代表服务端会话，
-- 在建立组合外键前清空，保留订单本身并恢复目录直购的无会话契约。
UPDATE orders AS o
SET session_id = NULL,
    source_turn_id = NULL
WHERE o.session_id IS NOT NULL
  AND NOT EXISTS (
      SELECT 1
      FROM sessions AS s
      WHERE s.id = o.session_id AND s.user_id = o.user_id
  );
