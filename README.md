...
| [Ubuntu 部署说明](deploy/README.md) | Docker、Nginx、HTTPS 和 GitHub Actions 部署 |
...
`13800000001` 和平台端 `13900000001`；手机号、账号和角色见 `sql/data.sql`。该实现不新增用户或权限表，
适合本地 POC，不包含 refresh token、服务端注销或多设备会话管理。

商家账号手机号如下，均使用默认密码 `12345678`：
